...
public class QueueFacadeMapper {
    ...
    public static QueueDomain toQueueDomain(QueueFacadeDTO facadeDTO) {
        return new QueueDomain(facadeDTO.queueNumber(), facadeDTO.userId(), facadeDTO.status(), facadeDTO.userQueueCount());
    }

    public static QueueFacadeDTO toQueueFacadeDTO(QueueDomain queueDomain) {
        return new QueueFacadeDTO(queueDomain.queueNumber(), queueDomain.userId(), queueDomain.status(), queueDomain.userQueueCount());
    }
}
